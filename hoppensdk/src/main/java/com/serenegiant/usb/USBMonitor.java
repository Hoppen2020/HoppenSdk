...
public final class USBMonitor {
	...
	private List<UsbDevice> noPermissionDevice = new ArrayList<>();
	...
	public interface OnDeviceConnectListener {
		 ...
		 * @param createNew
		 */
		public void onConnect(UsbDevice device, UsbControlBlock ctrlBlock, boolean createNew, int usbType);
		/**
	...
	public synchronized void register() throws IllegalStateException {
		...
		if (mPermissionIntent == null) {
			...
			mAsyncHandler.postDelayed(automaticPermission,1000);
		}
	}
	...
	public synchronized void unregister() throws IllegalStateException {
		...
		mDeviceCounts = 0;
		if (!destroyed) {
			mAsyncHandler.removeCallbacks(automaticPermission);
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
		...
		if (gettingPermission)return false;
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				//此处不会进，没有添加ACTION_USB_DEVICE_ATTACHED
				final UsbDevice device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
						...
						mAsyncHandler.post(new Runnable() {
							...
							public void run() {
								...
								noPermissionDevice.add(device);
								 //noPermissionDevice
								//Log.e("ABC",""+device.getVendorId()+"  "+device.getProductId());
							}
						});
					}
				}
	...
	private final Runnable automaticPermission = new Runnable() {
		@Override
		public  void run() {
			if (destroyed) return;
				if (noPermissionDevice.size()>0){
					//Log.e("XX",""+noPermissionDevice.size());
					boolean b = requestPermission(noPermissionDevice.get(0));
					//Log.e("XX",""+noPermissionDevice.size() +"  "+b);
				}
				mAsyncHandler.postDelayed(automaticPermission,1000);
		}
	};
	...
	private final void processConnect(final UsbDevice device) {
		...
		mAsyncHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (mOnDeviceConnectListener != null) {
					int type = DeviceType.DEVICE_TYPE_UNKNOW;
					for (DeviceFilter filter : mDeviceFilters){
						if (filter.mProductId == device.getProductId()&&filter.mVendorId==device.getVendorId()) {
							type = filter.deviceType;
							break;
						}
					}
					mOnDeviceConnectListener.onConnect(device, ctrlBlock, createNew,type);
				}
				noPermissionDevice.remove(device);
			}
		});
	}
